...
## Plain form — now the default

The most important change to the language since the original symbolic engine is **Plain form** — a natural-language surface that compiles English-like phrasings (`{SHOW user.name WITH LABEL}`, `{SHOW publications.title SORTED BY date JOINED BY ', '}`) into the same Polish-notation the engine runs internally. The goal was to make the language approachable to staff who would rather write "show each publication title joined by commas" than memorize that `{', ' publications.title}` does the same thing.

Plain was prototyped under the working name "PlainScript" in 2018–2020 — a tokenizer, parser, and translator that worked for a basic subset but never converged on a final syntax across multiple iterations. It landed in April 2026 as a companion layer, was promoted to the default export shortly after, and is now the front door for the library: `@uniweb/loom` exports `Loom` (the Plain-enabled class), and `@uniweb/loom/core` exports `LoomCore` (the raw symbolic engine) for users who explicitly want to skip the Plain parser. Loom is now one language with two surface forms — Plain and Compact — and a reader is expected to reach for whichever reads better for the expression in front of them. See the top-level [`README.md`](../README.md) for the current public API.
