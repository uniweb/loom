...
The most interesting piece added since the original symbolic engine is **Plain form** — a natural-language surface that compiles English-like phrasings (`{SHOW user.name WITH LABEL}`, `{SHOW publications.title SORTED BY date JOINED BY ', '}`) into the symbolic Polish-notation the engine runs internally. The goal was to make the language approachable to staff who would rather write "show each publication title joined by commas" than memorize that `{', ' publications.title}` does the same thing.

Plain was prototyped under the working name "PlainScript" in 2018–2020 — a tokenizer, parser, and translator that worked for a basic subset but never converged on a final syntax across multiple iterations. It landed in April 2026 as a companion layer shipped at the `@uniweb/loom/plain` subpath, and was promoted to the default export shortly after: `@uniweb/loom` now exports `Loom` (the Plain-enabled class) and `@uniweb/loom/core` exports `LoomCore` (the raw symbolic engine) for users who want to skip the Plain parser. See the top-level [`README.md`](../README.md) for the current public API.
