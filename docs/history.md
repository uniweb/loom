...
The most interesting unfinished piece is **Plain** — a natural-language layer that compiles English-like phrasings (`{SHOW user.name WITH LABEL}`, `{SHOW publications.title SORTED BY date JOINED BY ', '}`) into Loom's Polish-notation expressions. The goal is to make the language approachable to staff who would rather write "show each publication title joined by commas" than memorize that `{', ' publications.title}` does the same thing. It's the "saying it in plain language" companion to Loom's precise symbolic syntax.

Plain was prototyped under the working name "PlainScript" in 2018–2020 — a tokenizer, parser, and translator that worked for a basic subset but never converged on a final syntax across multiple iterations. The modern plan is to survey that material, settle on a unified syntax, and ship it as a subpath export at `@uniweb/loom/plain` (exporting a `Plain` class). See [`kb/plans/plain.md`](../../../kb/plans/plain.md) in the Uniweb workspace for the design document.
