...
Pre-existing limitations surfaced during stress-testing (not introduced by this refactor, all three investigated after the rename and resolved in follow-up commits):

- ✅ **Bug 1 — aggregation modifiers** (`{TOTAL OF grants.amount AS currency USD}`): the parser's `sum`/`average`/`count` branches in `parseExpression` returned raw AST nodes without calling `parseModifiers`, so `AS` / `WITH LABEL` / `WHERE` were left as unconsumed trailing tokens. Fixed by adding a `wrapWithModifiers(p, valueNode)` helper in the parser and hoisting WHERE handling out of `parseCountBody` into the uniform modifier path. The translator gained an aggregate-WHERE pre-pass in `translateShow` that emits the correct filter-then-aggregate Compact form — `SUM OF x.amount WHERE active` now compiles to `(++ (? x.active x.amount))`, `AVERAGE OF x.year WHERE refereed` to `(/ (++ (? x.refereed x.year)) (++!! x.refereed))`, etc. WHERE is a new capability on SUM/TOTAL/AVERAGE. Modifier ordering is irrelevant because the aggregate-WHERE rewrite runs before the main modifier loop: `COUNT OF x WHERE y AS number` and `COUNT OF x AS number WHERE y` produce identical Compact output.
- ✅ **Bug 2 — loom passthrough wrapping** (`{SHOW {+? 'Dr. ' title} WITH LABEL 'Name'}`): the translator's `'loom'` case returned `stripBraces(node.value)` unwrapped, so a parent modifier wrapping it as `(# -flag ${expr})` fragmented the multi-token inner into separate LoomCore args. Fixed by wrapping in parens when the inner has top-level whitespace (detected by a new `hasTopLevelWhitespace` helper), leaving single-token inners bare. Can't always wrap because LoomCore parses `(name)` as a function call to `name`, which would break `{SHOW {name} WITH LABEL}`-style templates.
- ❌ **Bug 3 — `{greet (SHOW name AS label)}`** was a false alarm. My probe used `new Loom()` with no `greet` function registered, so LoomCore correctly rejected the undefined name. With a registered function, the Plain pipeline compiles and renders this correctly.
